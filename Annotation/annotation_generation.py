...
class Config:
    ...
    detector_config_file: str = "../Dome-DETR/configs/dome/Dome-L-VisDrone.yml"  # Path to Dome-DETR config file (relative to GMIND-sdk)
    detector_checkpoint: str = "../Dome-DETR/Dome-L-VisDrone-best.pth"  # Path to Dome-DETR checkpoint (relative to GMIND-sdk)
    ...
    enable_depth_estimation: bool = False  # Enable 3D location computation using geometric ground plane intersection
    ...
    calibration_file: Optional[str] = "sensor_calibration.txt"  # Path to sensor_calibration.txt file (relative to Annotation/ directory, resolves to GMIND-sdk root)
    camera_name: Optional[str] = None  # Camera name in calibration file (e.g., "FLIR8.9mm"). If None, auto-extracted from video filename
    camera_matrix: Optional[np.ndarray] = None  # Camera intrinsics (3x3). If None and calibration_file is provided, will be parsed automatically
    dist_coeffs: Optional[np.ndarray] = None  # Distortion coefficients (5x1). If None and calibration_file is provided, will be parsed automatically
...
def parse_camera_intrinsics_from_calibration(
    ...
    try:
                    ...
                    if 'Focal_x' in intrinsics and 'Focal_y' in intrinsics:
                        ...
                        if fx > 0 and fy > 0:  # Valid intrinsics
                            ...
                            if 'Dist_1' in intrinsics or 'Distortion_1' in intrinsics:
                                ...
                                alt_keys = ['Distortion_1', 'Distortion_2', 'Distortion_3', 'Distortion_4', 'Distortion_5']
                                dist_values = []
                                ...
                                if any(abs(v) > 1e-6 for v in dist_values):
                                    ...
                                    print(f"✓ Parsed distortion coefficients: {dist_coeffs}", flush=True)
                            
                            print(f"✓ Parsed camera intrinsics for '{name}' from {calib_path}", flush=True)
                            print(f"  fx={fx:.2f}, fy={fy:.2f}, cx={cx:.2f}, cy={cy:.2f}", flush=True)
        ...
        if matched_camera is None:
            print(f"⚠️  Warning: No matching camera found in calibration file: {calib_path}", flush=True)
            if search_camera_name:
                print(f"  Searched for: '{search_camera_name}'", flush=True)
            print(f"  Available cameras: {all_cameras}", flush=True)
        else:
            print(f"⚠️  Warning: Could not parse intrinsics for camera '{matched_camera}' from {calib_path}", flush=True)
        return None, None
...
class TrackedObject:
    ...
    frame_positions: Dict[int, np.ndarray] = field(default_factory=dict)  # {frame_id: bbox} for detected positions
    interpolated_positions: Dict[int, np.ndarray] = field(default_factory=dict)  # {frame_id: bbox} for interpolated positions
    ...
    frame_3d_locations: Dict[int, np.ndarray] = field(default_factory=dict)  # {frame_id: [X, Y, Z]} for detected 3D positions (world coords)
    interpolated_3d_locations: Dict[int, np.ndarray] = field(default_factory=dict)  # {frame_id: [X, Y, Z]} for interpolated 3D positions (world coords)
    last_known_3d_location: Optional[np.ndarray] = None  # Last known 3D location for interpolation (world coords)
    ...
    def update(self, bbox: np.ndarray, frame_id: int, confidence: float = 1.0, 
        ...
        if self.last_known_frame >= 0 and frame_id > self.last_known_frame + 1:
            ...
            self._interpolate_between_frames(
                self.last_known_frame, self.last_known_bbox, 
                frame_id, bbox,
                self.last_known_3d_location, location_3d  # Also interpolate 3D locations
            )
    ...
    def get_3d_location_for_frame(self, frame_id: int) -> Optional[np.ndarray]:
        ...
        if frame_id in self.frame_3d_locations:
            ...
            if loc_3d is not None and len(loc_3d) == 3 and not np.any(np.isnan(loc_3d)) and not np.any(np.isinf(loc_3d)):
                return loc_3d
    ...
    def _interpolate_between_frames(self, start_frame: int, start_bbox: np.ndarray,
...
class ObjectDetector:
    ...
    def _load_model(self):
        ...
        if self.config.detector_model.lower() == 'dome-detr' or self.config.detector_model.lower().startswith('dome'):
            ...
            try:
                ...
                dome_detr_dir = os.path.dirname(os.path.dirname(os.path.dirname(config_file)))  # Go up from configs/dome/Dome-L-VisDrone.yml to Dome-DETR root
            ...
            try:
                ...
                self.model = get_model('yolo12x', num_classes=80, pretrained=True, backend='ultralytics')
                if hasattr(self.model, 'to'):
                    self.model.to(self.device)
                if hasattr(self.model, 'eval'):
                    self.model.eval()
        ...
        if len(results) > 0 and hasattr(results[0], 'boxes'):
            ...
            if boxes is not None and len(boxes) > 0:
                ...
                for i in range(len(boxes_data)):
                    ...
                    x1, y1, x2, y2 = int(boxes_data[i][0]), int(boxes_data[i][1]), int(boxes_data[i][2]), int(boxes_data[i][3])
    ...
    def _parse_mmdetection_results(self, results, conf_threshold: float) -> List[Detection]:
        ...
        use_visdrone = (self.config.detector_model.lower() == 'dome-detr' or 
                       self.config.detector_model.lower().startswith('dome'))
        class_names = VISDRONE_CLASSES if use_visdrone else COCO_CLASSES
        valid_class_ids = [0, 1, 2, 3] if use_visdrone else [0, 1, 2]  # VisDrone: pedestrian, people, bicycle, car
    ...
    def _parse_dome_detr_results(self, labels, boxes, scores, conf_threshold: float) -> List[Detection]:
        ...
        try:
            ...
            labels_batch = np.array(labels_batch) if not isinstance(labels_batch, np.ndarray) else labels_batch
            boxes_batch = np.array(boxes_batch) if not isinstance(boxes_batch, np.ndarray) else boxes_batch
            scores_batch = np.array(scores_batch) if not isinstance(scores_batch, np.ndarray) else scores_batch
            ...
            if len(scores_batch.shape) > 1 and scores_batch.shape[1] > 1:
                ...
                max_classes = labels_batch.flatten() if len(labels_batch.shape) > 0 else labels_batch
            ...
            for track in tracked:
                ...
                if existing_track is not None:
                    existing_track.update(
                        ...
                        frame_id, score, class_id=class_id, class_name=class_name
                    )
...
def save_coco_annotations(
    ...
    categories = [
        ...
        for class_id in sorted(class_ids_seen) if class_id < len(COCO_CLASSES)
    ]
    ...
    for frame_idx in all_frame_indices:
        ...
        for track_id, track in all_unique_tracks.items():
            ...
            if location_3d is not None and len(location_3d) == 3:
                annotation_data["location_3d"] = [float(location_3d[0]), float(location_3d[1]), float(location_3d[2])]
...
def process_video(video_path: str, config: Config):
    ...
    if config.process_every_n_frames == 1 and config.target_processing_fps > 0 and fps > 0:
        ...
        print(f"Auto-calculated frame skip: {calculated_skip} (targeting {config.target_processing_fps} fps)", flush=True)
    ...
    if config.enable_depth_estimation and camera_matrix is None and config.calibration_file is not None:
        ...
        if calib_path.startswith('H:\\') or calib_path.startswith('H:/'):
            calib_path = calib_path.replace('H:\\', '/mnt/h/').replace('H:/', '/mnt/h/').replace('\\', '/')
    ...
    if config.enable_depth_estimation:
        print(f"✓ 3D location computation enabled: Geometric (ground plane intersection)", flush=True)
        print(f"  Camera height: {config.camera_height}m, Pitch: {config.camera_pitch_deg}°", flush=True)
        if config.camera_matrix is not None:
            print(f"  Using camera intrinsics for geometric projection", flush=True)
        else:
            print(f"  ⚠️  Warning: Camera intrinsics not available, geometric depth will fail", flush=True)
    ...
    while True:
        ...
        if config.enable_depth_estimation:
            if config.camera_matrix is not None:
                ...
                try:
                    from .footpoint_to_ground import bbox_to_3d_geometric_robust as bbox_to_3d_geometric
                except ImportError:
                    from footpoint_to_ground import bbox_to_3d_geometric_robust as bbox_to_3d_geometric
                    ...
                    if track.last_known_frame >= 0 and frame_idx > track.last_known_frame + 1:
                        ...
                        if prev_3d is not None and len(prev_3d) == 3:
                            ...
                            for gap_frame in range(track.last_known_frame + 1, frame_idx):
                                alpha = (gap_frame - track.last_known_frame) / (frame_idx - track.last_known_frame)
                                alpha = np.clip(alpha, 0.0, 1.0)
    ...
    if config.save_annotations and len(tracked_objects_by_frame) > 0:
        ...
        save_coco_annotations(video_path, tracked_objects_by_frame, width, height, output_file, config, tracker=tracker, total_frames=total_frames)
