...
class Config:
    ...
    process_every_n_frames: int = (
        5  # Process every Nth frame (1 = all frames, 2 = every other frame, etc.)
    )
    ...
    # Object Detection
    detector_model: str = (
        "rtdetr-x"  # Model to use: "yolo11x", "yolov8x", "rtdetr-l", "rtdetr-x", "maskrcnn_resnet50_fpn_v2"
    )
    ...
    static_detection_sample_frames: int = (
        20  # Number of random frames to sample for static object detection
    )
    static_detection_iou_threshold: float = (
        0.5  # IoU threshold for grouping static objects across frames
    )
    use_dual_detector: bool = (
        True  # Use YOLOv11x for full frame + RT-DETR-X for regions, then merge with NMS
    )
    full_frame_detection_frequency: int = (
        0  # DEPRECATED: Static objects are detected via static_detection_sample_frames. Set to 0 to disable.
    )
    nms_threshold: float = (
        0.2  # IoU threshold for NMS when merging detections (lower = more aggressive)
    )
    nms_nested_ratio: float = (
        0.25  # If smaller box area is < this ratio of larger box, merge nested detections
    )
    nms_nested_iou: float = (
        0.5  # If IoU > this and one box is much smaller, merge (for partial nesting)
    )
    filter_by_aspect_ratio: bool = (
        True  # Filter detections by aspect ratio based on expected object shapes
    )
    ...
    aspect_ratio_bounds: Dict[int, Tuple[float, float]] = field(
        default_factory=lambda: {
            0: (
                0.2,
                2.0,
            ),  # Person: width/height between 0.2 and 2.0 (tall, lenient for distortion/poses)
            1: (
                0.5,
                4.0,
            ),  # Bicycle: width/height between 0.5 and 4.0 (varies by angle, lenient for distortion)
            2: (
                0.8,
                5.0,
            ),  # Car: width/height between 0.8 and 5.0 (wider than tall, lenient for distortion)
        }
    )
    min_box_size: int = (
        5  # Minimum width or height in pixels (filters invalid/suspiciously small boxes)
    )
    ...
    tracking_det_thresh: float = (
        detector_conf_threshold  # Detection confidence threshold for OC-SORT (should match detector_conf_threshold)
    )
    ...
    tracking_asso_func: str = (
        "giou"  # Association function: "iou", "giou", "ciou", "diou", "ct_dist" (GIoU handles scale changes better)
    )
    enable_bicycle_pedestrian_association: bool = (
        True  # Link bicycles and pedestrians when nearby for robust tracking
    )
    enable_backward_validation: bool = (
        False  # If True, include early frames of tracks that become valid later (retroactive validation)
    )
    max_speed_pixels_per_frame: float = (
        200.0  # Maximum allowed movement in pixels per frame (filters unrealistic jumps)
    )
    filter_static_dynamic_objects: bool = (
        True  # Filter out dynamic tracks that are actually static (BGS noise)
    )
    static_dynamic_max_movement: float = (
        20.0  # Maximum total movement in pixels for a track to be considered static
    )
    static_dynamic_min_frames: int = (
        10  # Minimum number of frames a track must be detected before checking if it's static
    )
...
class TrackedObject:
    ...
    extrapolated_frames: Any = field(
        default_factory=list
    )  # Frame IDs where this track was extrapolated
    ...
    extrapolated_positions: Any = field(
        default_factory=dict
    )  # {frame_id: bbox} for extrapolated positions
    interpolated_positions: Any = field(
        default_factory=dict
    )  # {frame_id: bbox} for interpolated positions
    frame_positions: Any = field(
        default_factory=dict
    )  # {frame_id: bbox} for actual detected positions at each frame
    # Tracking statistics
    ...
    class_history: Any = field(
        default_factory=list
    )  # History of class_id from all detections for this track
    # Bicycle-pedestrian association
    associated_track_ids: Any = field(
        default_factory=set
    )  # Track IDs of associated objects (e.g., bicycle-person pairs)
        ...
        if class_id is not None:
            ...
            if len(self.class_history) > 0:
                ...
                if hasattr(current_module, "COCO_CLASSES"):
                    ...
                    self.class_name = (
                        COCO_CLASSES[self.class_id]
                        if 0 <= self.class_id < len(COCO_CLASSES)
                        else f"class_{self.class_id}"
                    )
                else:
    ...
    def get_bbox_for_frame(self, frame_id: int) -> Optional[np.ndarray]:
        ...
        has_interpolated = (
            frame_id in self.interpolated_positions if self.interpolated_positions else False
        )
        ...
        if len(all_available_frames) > 0:
            ...
            if before_bbox is not None and after_bbox is not None:
                ...
                if (
                    interpolated_bbox is not None
                    and len(interpolated_bbox) == 4
                    and not np.any(np.isnan(interpolated_bbox))
                    and not np.any(np.isinf(interpolated_bbox))
                ):
        ...
        if self.frame_id >= 0 and abs(frame_id - self.frame_id) <= 1:
            if (
                ...
                and len(self.bbox) == 4
                and not np.any(np.isnan(self.bbox))
                and not np.any(np.isinf(self.bbox))
            ):
                return self.bbox.copy()
...
class ForegroundSegmenter:
    ...
    def __init__(self, config: Config):
        ...
        self.morph_kernel = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE, (config.fg_seg_morph_kernel_size, config.fg_seg_morph_kernel_size)
        )
        ...
        if self.config.fg_seg_method == "BGS":
            ...
            if self.config.fg_seg_learning_rate >= 0:
                fg_mask = self.bg_subtractor.apply(
                    frame, learningRate=self.config.fg_seg_learning_rate
                )
            else:
...
class BlobExtractor:
    ...
    def extract(self, fg_mask: np.ndarray) -> List[Dict[str, Any]]:
        ...
        num_labels, labeled_mask, stats, centroids = cv2.connectedComponentsWithStats(
            fg_mask, connectivity=self.config.cc_connectivity
        )
...
class ObjectDetector:
    ...
    def _load_model(self):
        ...
        if self.config.use_dual_detector:
            ...
            try:
                ...
                self.model_full_frame = get_model(
                    "yolo11x", num_classes=80, pretrained=True, backend="ultralytics"
                )
            ...
            try:
                ...
                self.model = get_model(
                    "rtdetr-x", num_classes=80, pretrained=True, backend="ultralytics"
                )
                ...
                try:
                    self.model = get_model(
                        "rtdetr-l", num_classes=80, pretrained=True, backend="ultralytics"
                    )
                    if hasattr(self.model, "to"):
                        self.model.to(self.device)
                    if hasattr(self.model, "eval"):
        ...
        try:
            ...
            self.model = get_model(
                "yolov8x", num_classes=80, pretrained=True, backend="ultralytics"
            )
            if hasattr(self.model, "to"):
                self.model.to(self.device)
            if hasattr(self.model, "eval"):
        ...
        try:
            ...
            self.model = get_model(
                "maskrcnn_resnet50_fpn_v2", num_classes=91, pretrained=True, backend="torchvision"
            )
            self.model.to(self.device)
            self.model.eval()
    ...
    def _compute_iou(box1: np.ndarray, box2: np.ndarray) -> float:
        ...
        if (
            np.any(np.isnan(box1))
            or np.any(np.isinf(box1))
            or np.any(np.isnan(box2))
            or np.any(np.isinf(box2))
        ):
            return 0.0
    ...
    def _check_aspect_ratio(
        bbox: np.ndarray, class_id: int, aspect_ratio_bounds: Dict[int, Tuple[float, float]]
    ) -> bool:
        """
    ...
    @staticmethod
    def _is_box_within(
        box_inner: np.ndarray, box_outer: np.ndarray, min_overlap: float = 0.8
    ) -> bool:
        ...
        if (
            np.any(np.isnan(box_inner))
            or np.any(np.isinf(box_inner))
            or np.any(np.isnan(box_outer))
            or np.any(np.isinf(box_outer))
        ):
            return False
        ...
        while len(sorted_dets) > 0:
            ...
            for det in sorted_dets:
                if det.class_id != keep_det.class_id:
                    ...
                    is_high_confidence = (
                        det.confidence > high_confidence_threshold
                        or keep_det.confidence > high_confidence_threshold
                    )
                    ...
                    nested_ratio = (
                        ...
                        else self.config.nms_nested_ratio
                    )
                    ...
                    if not is_high_confidence:
                        is_partial_nested = self._is_partial_nested(
                            keep_det.bbox,
                            det.bbox,
    ...
    @staticmethod
    def _is_nested_box(
        box1: np.ndarray, box2: np.ndarray, area_ratio_threshold: float = 0.5
    ) -> bool:
        """
            ...
            if len(blobs) > 0:
                try:
                    for blob in blobs:
                        ...
                        roi_resized, scale, pad_x, pad_y = self._resize_with_aspect_ratio(
                            roi, target_size=640
                        )

                        # Run RT-DETR-X on region
                        results_region = self.model.predict(
                            roi_resized, conf=conf_threshold, verbose=False
                        )
                        region_dets = self._parse_ultralytics_results(
                            results_region, conf_threshold
                        )
...
class Tracker:
        ...
        if self.config.tracking_method == "OC-SORT":
            try:
                ...
                if len(detections_xyxy) > 0:
                    detections_xyxy[:, 2] = (
                        detections_xyxy[:, 0] + detections_xyxy[:, 2]
                    )  # x2 = x1 + w
                    detections_xyxy[:, 3] = (
                        detections_xyxy[:, 1] + detections_xyxy[:, 3]
                    )  # y2 = y1 + h
    ...
    def _update_simple(self, detections: List[Detection], frame_id: int) -> List[TrackedObject]:
        ...
        if len(detections) == 0:
            ...
            # Remove old tracks
            self.tracks = [
                t for t in self.tracks if (frame_id - t.frame_id) < self.config.tracking_max_age
            ]
            return self.tracks
        ...
        self.tracks = [t for t in self.tracks if t.time_since_update < self.config.tracking_max_age]
...
def detect_static_objects(
    ...
    print(
        f"\nDetecting static objects: sampling {config.static_detection_sample_frames} random frames...",
        flush=True,
    )
    ...
    sample_frames = sorted(
        random.sample(range(total_frames), min(config.static_detection_sample_frames, total_frames))
    )
    ...
    for sample_frame_idx in sample_frames:
        ...
        try:
            results = object_detector.model_full_frame.predict(
                frame, conf=config.detector_conf_threshold, verbose=False
            )
            detections = object_detector._parse_ultralytics_results(
                results, config.detector_conf_threshold
            )
...
def save_coco_annotations(
    ...
    for class_id in sorted(class_ids_seen):
        class_name = (
            COCO_CLASSES[class_id] if 0 <= class_id < len(COCO_CLASSES) else f"class_{class_id}"
        )
        categories.append(
            {"id": class_id + 1, "name": class_name, "supercategory": "none"}  # COCO uses 1-indexed
        )
    ...
    for frame_idx in sorted(all_frame_indices):
            ...
            for track_id in valid_track_ids:
                if track_id not in frame_track_ids:
                    ...
                    if obj is not None:
                        ...
                        if (
                            test_bbox is not None
                            and len(test_bbox) == 4
                            and not np.any(np.isnan(test_bbox))
                            and not np.any(np.isinf(test_bbox))
                        ):
        ...
        for track_id in sorted(frame_track_ids):
            ...
            has_interpolated = (
                frame_idx in obj.interpolated_positions if obj.interpolated_positions else False
            )
            ...
            is_extrapolated = (
                frame_idx in obj.extrapolated_frames if obj.extrapolated_frames else False
            )
...
def process_video(video_path: str, config: Config):
    ...
    while True:
        ...
        has_active_tracks = (
            any(track.time_since_update < config.tracking_max_age for track in tracker.tracks)
            if len(tracker.tracks) > 0
            else False
        )
            ...
            for bicycle in bicycles:
                ...
                for person in pedestrians:
                    ...
                    if (
                        ...
                        and center_distance_x < max(bike_w, person_w) * 1.5
                        and center_distance_y < max(bike_h, person_h) * 1.5
                    ):
        ...
        if (
            config.save_annotations
            and last_processed_frame >= 0
            and config.process_every_n_frames > 1
            and len(filtered_tracked_objects) > 0
        ):
                ...
                if len(last_frame_track_map) > 0 or len(filtered_tracked_objects) > 0:
                    ...
                    for skipped_frame in skipped_frames:
                        ...
                        skipped_frame_track_map[skipped_frame] = {
                            track.track_id: track for track in skipped_frame_tracks
                        }
                    ...
                    for current_track in filtered_tracked_objects:
                        ...
                        if track_id in last_frame_track_map:
                            ...
                            if (
                                ...
                                and not np.any(np.isnan(last_bbox))
                                and not np.any(np.isnan(current_bbox))
                                and not np.any(np.isinf(last_bbox))
                                and not np.any(np.isinf(current_bbox))
                            ):
                                ...
                                for skipped_frame in skipped_frames:
                                    ...
                                    if track_id not in skipped_track_map:
                                        ...
                                        if not any(
                                            t.track_id == track_id
                                            for t in tracked_objects_by_frame[skipped_frame]
                                        ):
                                            tracked_objects_by_frame[skipped_frame].append(
                                                current_track
                                            )
                                    else:
                                        ...
                                        for sf in skipped_frames:
                                            if sf in current_track.interpolated_positions:
                                                skipped_track.interpolated_positions[sf] = (
                                                    current_track.interpolated_positions[sf].copy()
                                                )
        ...
        if not config.headless:
            ...
            if config.debug_visualization:
                ...
                if len(blobs) > 0:
                    ...
                    for i, blob in enumerate(blobs):
                        ...
                        if blob_mask.shape != fg_mask_vis.shape[:2]:
                            print(
                                f"Warning: Blob {i} mask shape {blob_mask.shape} != image shape {fg_mask_vis.shape[:2]}"
                            )
                            continue

                        # Generate distinct bright color for each blob using HSV
                        hue = int((i * 180 / max(len(blobs), 1)) % 180)
                        blob_color_bgr = cv2.cvtColor(
                        ...
                        cv2.rectangle(
                            fg_mask_vis, (x1_orig, y1_orig), (x2_orig, y2_orig), (255, 0, 0), 3
                        )
                        cv2.putText(
                            fg_mask_vis,
                            f"Orig {i}",
                            (x1_orig, max(15, y1_orig - 5)),
                        ...
                        cv2.putText(
                            fg_mask_vis,
                            f"Exp {i} (YOLO)",
                            (x1, min(fg_mask_vis.shape[0] - 5, y2 + 20)),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.7,
                            (0, 255, 0),
                            2,
                ...
                cv2.putText(
                    fg_mask_vis,
                    f"Blobs: {len(blobs)} | Frame: {frame_idx}",
                    (10, 60),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,
                    (255, 255, 255),
                ...
                if len(blobs) > 0:
                    cv2.putText(
                        fg_mask_vis,
                        "Blue: Original | Green: Expanded (YOLO)",
                        (10, 90),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.7,
                        (255, 255, 255),
                ...
                cv2.putText(
                    dl_regions_vis,
                    "DL Regions - YOLO Detection Areas",
                    (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1.0,
                    (255, 255, 255),
                ...
                cv2.putText(
                    dl_regions_vis,
                    f"Blob Regions: {len(blobs)} | Detections: {len(detections)} | Frame: {frame_idx}",
                    (10, 60),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,
                    (255, 255, 255),
                ...
                if config.detect_static_objects:
                    cv2.putText(
                        dl_regions_vis,
                        "Static Detection: ENABLED (Full Frame)",
                        (10, 90),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.7,
                        (0, 255, 0),
                    ...
                    cv2.putText(
                        dl_regions_vis,
                        "Static Detection: DISABLED (FG Only)",
                        (10, 90),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.7,
                        (0, 0, 255),
                ...
                cv2.putText(
                    static_objects_vis,
                    f"Frame: {frame_idx} | Static Objects: {len(static_objects) if static_objects else 0}",
                    (10, 60),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,
                    (255, 255, 255),
                ...
                cv2.putText(
                    all_objects_vis,
                    "All Valid Objects (Static + Dynamic)",
                    (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1.0,
                    (255, 255, 255),
            ...
            track_history = {
                tid: hist for tid, hist in track_history.items() if tid in active_track_ids
            }
