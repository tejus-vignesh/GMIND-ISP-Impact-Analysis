...
def undistort_bbox_points(
    bbox: np.ndarray, camera_matrix: np.ndarray, dist_coeffs: Optional[np.ndarray] = None
) -> np.ndarray:
    """
    ...
    return np.array(
        [undistorted[0, 0], undistorted[0, 1], undistorted[1, 0], undistorted[1, 1]],
        dtype=np.float32,
    )
...
def get_representative_pixel(
    bbox: np.ndarray, method: str = "bottom_center", num_samples: int = 1
) -> np.ndarray:
    """
...
def pixel_to_normalized_coords(pixel: np.ndarray, camera_matrix: np.ndarray) -> np.ndarray:
    """
...
def build_rotation_cam_to_world(
    pitch_deg: float = 0.0, roll_deg: float = 0.0, yaw_deg: float = 0.0
) -> np.ndarray:
    """
...
def rotate_ray_to_world(ray_cam: np.ndarray, R_cam_to_world: np.ndarray) -> np.ndarray:
    """
...
def bbox_to_3d_geometric_robust(
    ...
    R_cam_to_world = build_rotation_cam_to_world(
        pitch_deg=camera_pitch_deg, roll_deg=camera_roll_deg, yaw_deg=camera_yaw_deg
    )
