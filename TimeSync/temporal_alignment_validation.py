...
def plot_3d_locations_interactive(
    ...
    print(f"  Azimuth (XY plane): {azimuths.min():.1f}° to {azimuths.max():.1f}° (span: {azimuths.max() - azimuths.min():.1f}°)")
    print(f"  Elevation (Z): {elevations.min():.1f}° to {elevations.max():.1f}° (span: {elevations.max() - elevations.min():.1f}°)")
    ...
    if var_ratio < 0.01:
        print(f"  WARNING: Points appear to be collinear (low variance in {'XYZ'[min_var_idx]} dimension)")
...
def plot_3d_locations_over_time(
    points_3d: np.ndarray,
    metadata: List[Dict],
    title: str = "3D Object Locations Over Time"
):
    """
    ...
    for cat_id in sorted(unique_categories):
        cat_name = next((m['category_name'] for m in metadata if m['category_id'] == cat_id), 'unknown')
    ...
    def animate(frame_idx):
        ...
        # Update frame text
        frame_text.set_text(f'Frame: {current_frame} / {sorted_frames[-1]}\n'
    ...
    animation = FuncAnimation(fig, animate, frames=len(sorted_frames), 
...
def plot_xy_plane_over_time_multicam(
    ...
    def find_and_load_video(camera_name: str, annotation_dir: Optional[str]) -> Optional[cv2.VideoCapture]:
    ...
    def build_video_frame_mapping(camera_name: str, sorted_frames: List[int], metadata: List[Dict]) -> Dict[int, int]:
        """
    ...
    if video_enabled and annotation_dir is not None:
        ...
        for camera_name in points_3d_dict.keys():
            ...
            if cap is not None:
                ...
                video_frame_mappings[camera_name] = build_video_frame_mapping(camera_name, sorted_frames, metadata)
    ...
    ax.scatter([0], [0], c='red', s=200, marker='*', 
              label='Origin (Camera)', alpha=0.9, edgecolors='black', linewidths=2, zorder=10)
    ...
    frame_text = ax.text(0.5, 0.02, '', transform=ax.transAxes, 
    ...
    alignment_text_obj = ax.text(0.98, 0.98, '', transform=ax.transAxes,
    ...
    region_rect = plt.Rectangle(
        ...
        linewidth=2, edgecolor='cyan', facecolor='none', linestyle='--', alpha=0.7, zorder=1
    )
    ...
    static_object_threshold_meters = 2.0  # Maximum position variance for an object to be considered static
    ...
    static_object_tracks = identify_static_objects(points_by_frame_dict, static_object_threshold_meters)
    
    if exclude_static_objects and len(static_object_tracks) > 0:
        print(f"   🔍 Identified {len(static_object_tracks)} static object tracks (threshold: {static_object_threshold_meters}m)")
    ...
    def compute_alignment_metrics_from_camera_points(camera_points_internal, max_neighbor_dist=8.0, exclude_static=False):
        """
        ...
        for cat_id in camera_points_internal['FLIR 8.9MP']:
            for point, meta in camera_points_internal['FLIR 8.9MP'][cat_id]:
                ...
                # Filter by region
                if not (alignment_region_x_min <= x <= alignment_region_x_max and
                        alignment_region_y_min <= y <= alignment_region_y_max):
                    continue
        ...
        if len(neighbor_distances) > 0:
            ...
            if len(neighbor_distances) >= 5:  # Need at least 5 values to trim meaningfully
                ...
                trimmed_distances = sorted_distances[trim_count:-trim_count] if trim_count > 0 else sorted_distances
    ...
    def update_video_displays(camera_points_current: Dict, matched_pairs_current: List[Dict]):
        ...
        for camera_name, cap in video_captures.items():
            ...
            video_frame_idx = video_frame_mappings[camera_name].get(target_frame_id, target_frame_id)
            ...
            if camera_name in camera_points_current:
                ...
                }.get(camera_name, (128, 128, 128))  # Gray default
                ...
                for cat_id, objects in camera_points_current[camera_name].items():
                    for point, meta in objects:
                        ...
                        if len(bbox) >= 4:
                            ...
                            if track_id >= 0 and (camera_name, 'track', track_id) in matched_objects:
                                matched = True
                                match_id, match_color = matched_objects[(camera_name, 'track', track_id)]
                            elif ann_id >= 0 and (camera_name, 'ann', ann_id) in matched_objects:
                                matched = True
                                match_id, match_color = matched_objects[(camera_name, 'ann', ann_id)]
                            ...
                            cv2.circle(frame, (footpoint_x, footpoint_y), footpoint_radius, footpoint_color, footpoint_thickness)
                            ...
                            if matched:
                                ...
                                cv2.line(frame, (footpoint_x, footpoint_y), (footpoint_x, line_end_y), 
                                        match_color, 3)
                                
                                # Draw match ID number below the line
                                match_id_text = f"M{match_id}"
                                text_size, _ = cv2.getTextSize(match_id_text, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
                                ...
                                cv2.rectangle(frame, 
                                            ...
                                            match_color, -1)
                                cv2.putText(frame, match_id_text, (text_x, text_y),
                ...
                cv2.putText(frame, info_text_video, (10, 30), 
    ...
    def update_frame():
        ...
        max_neighbor_distance = 8.0  # 8m hard cutoff - objects further apart are not considered matches
        ...
        if False:  # Original inline code replaced with function call above
            ...
            for cat_id in camera_points['FLIR 8.9MP']:
                for point, meta in camera_points['FLIR 8.9MP'][cat_id]:
                    ...
                    # Filter by region
                    if (alignment_region_x_min <= x <= alignment_region_x_max and
                        alignment_region_y_min <= y <= alignment_region_y_max):
                        flir89_points.append({
                            'point': np.array([x, y]),
                            'cat_id': cat_id,
            ...
            if len(flir89_points) > 0 and len(other_camera_points) > 0:
                ...
                matched_other_keys = set()  # Track which other camera objects are matched: (camera_name, index)
        ...
        for camera_name in camera_names_sorted:
            ...
            for cat_id, cat_name in sorted_categories:
                if cat_id in camera_points[camera_name] and len(camera_points[camera_name][cat_id]) > 0:
                    ...
                    scatter = ax.scatter(points[:, 0], points[:, 1],
                                       c=cam_color, marker=marker, s=80, 
        ...
        for pair in matched_pairs:
            ...
            line = ax.plot([flir89_pt[0], matched_pt[0]], 
        ...
        if alignment_stats:
            ...
            alignment_text_obj.set_bbox(dict(boxstyle='round', facecolor='lightgreen', alpha=0.9, edgecolor='black'))
            alignment_text_obj.set_text('\n'.join(stats_lines))
        else:
            alignment_text_obj.set_bbox(dict(boxstyle='round', facecolor='lightyellow', alpha=0.9, edgecolor='black'))
    ...
    legend_elements.append(plt.Line2D([0], [0], marker='*', color='w', 
                                     ...
                                     label='Origin (Camera)', linestyle='None'))
    ...
    for cam_name in camera_names_sorted:
        ...
        legend_elements.append(plt.Line2D([0], [0], marker='o', color='w', 
                                         ...
                                         label=f'{cam_name}', linestyle='None'))
    ...
    for cat_id, cat_name in sorted_categories:
        ...
        legend_elements.append(plt.Line2D([0], [0], marker=marker, color='w', 
                                         ...
                                         markeredgecolor='black', markeredgewidth=1,
                                         label=f'{cat_name}', linestyle='None'))
    ...
    def on_key_press(event):
        ...
        # Search adjusts the active camera's offset (or all other cameras if FLIR 8.9MP is active)
            ...
            max_forward = min(max_search_range, len(search_camera_sorted_frames) - 1 - current_base_frame_idx)
            ...
            if current_base_frame_idx < max_search_range:
                ...
                print(f"   At beginning (index {current_base_frame_idx}), searching forward: 0 to +{max_forward} frames")
                ...
                print(f"   Near end (index {current_base_frame_idx}), searching backward: -{max_backward} to 0 frames")
            else:
                # In the middle - search both directions
                search_range = range(-max_backward, max_forward + 1)
                print(f"   Searching range: -{max_backward} to +{max_forward} frames (from index {current_base_frame_idx})")
            ...
            for offset_delta in search_range:
                ...
                for camera_name in camera_names_sorted:
                    ...
                    if target_frame_id_test not in points_by_frame_test:
                        ...
                        if available_frames_test:
                            actual_frame_id_test = min(available_frames_test, key=lambda x: abs(x - target_frame_id_test))
                ...
                if test_stats:
                    ...
                    metric_value = test_stats.get('median') if test_stats.get('median') is not None else float('inf')
            ...
            if len(offset_values) > 0:
                ...
                if valid_median_indices:
                    ...
                    plot_ax.plot(valid_offsets_median, valid_median_vals, 'o-', color='blue', label='Median', linewidth=2, markersize=6)
                    ...
                    if best_metric_type == 'median' and best_offset in valid_offsets_median:
                        ...
                        plot_ax.plot(best_offset, valid_median_vals[best_idx], 'rs', markersize=12, label=f'Best (median={valid_median_vals[best_idx]:.3f}m)', zorder=10)
                ...
                if valid_match_indices:
                    ...
                    plot_ax2.plot(valid_offsets_match, valid_match_counts, '^-', color='orange', label='Match count', linewidth=2, markersize=6, alpha=0.7)
                
                # Set labels and styling
                plot_ax.set_xlabel('Offset (frames)', fontsize=12, fontweight='bold')
                plot_ax.set_ylabel('Distance (meters)', fontsize=12, fontweight='bold', color='black')
                plot_ax2.set_ylabel('Number of Matches', fontsize=12, fontweight='bold', color='orange')
                plot_ax2.tick_params(axis='y', labelcolor='orange')
                plot_ax.set_title(f'Alignment Metrics vs Offset ({search_camera})', fontsize=14, fontweight='bold')
                ...
                best_match_count = match_counts[offset_values.index(best_offset)] if best_offset in offset_values else 0
                plot_ax.text(0.02, 0.98, f'Best offset: {best_offset:+d}\nMetric: {best_metric_type} = {best_metric_value:.3f}m\nMatches: {best_match_count}',
                           transform=plot_ax.transAxes, fontsize=10, verticalalignment='top',
            ...
            if best_offset != original_offset:
                ...
                print(f"✅ Found best alignment for {search_camera} at offset: {best_offset:+d} (metric: {best_metric_type} = {best_metric_value:.3f}m)")
                ...
                if len(flir89_sorted_frames) > 0 and len(search_sorted_frames) > 0:
                    ...
                    flir89_base_frame_id = flir89_sorted_frames[flir89_frame_idx] if flir89_frame_idx < len(flir89_sorted_frames) else flir89_sorted_frames[0]
                    search_base_frame_id = search_sorted_frames[search_frame_idx] if search_frame_idx < len(search_sorted_frames) else search_sorted_frames[0]
                    ...
                    print(f"\n📊 Frame Alignment:")
                    print(f"   FLIR 8.9MP: frame index {flir89_frame_idx} → frame ID {flir89_actual_frame}")
                    print(f"   {search_camera}: frame index {search_frame_idx} → frame ID {search_actual_frame}")
                
                if frame_results:
                    print(f"\n   Top 3 offsets:")
                    sorted_results = sorted([r for r in frame_results if r['median'] is not None],
                                           key=lambda x: x.get('median', float('inf')))
                    for i, result in enumerate(sorted_results[:3]):
                        metric_val = result.get('median')
                        print(f"     {i+1}. Offset {result['offset']:+d}: median={metric_val:.3f}m ({result['count']} matches)")
            else:
                camera_frame_offsets[search_camera] = original_offset  # Restore if no change
                print(f"✓ Current {search_camera} offset is already best: {best_offset:+d} (metric: {best_metric_type} = {best_metric_value:.3f}m)")
    ...
    for camera_name in camera_names_sorted:
        ...
        sorted_frames_cam = sorted_frames_dict[camera_name]
        frame_range = f"{sorted_frames_cam[0]}-{sorted_frames_cam[-1]}" if sorted_frames_cam else "N/A"
    ...
    print(f"  'm' - Toggle static object exclusion (currently: {'ON' if exclude_static_objects else 'OFF'})")
...
def main():
    ...
    parser.add_argument(
        '--plot_time',
        action='store_true',
        help='Plot objects appearing over time (animated)'
    )
    parser.add_argument(
        '--plot_xy',
        action='store_true',
    ...
    if args.plot_xy:
        ...
        plot_xy_plane_over_time_multicam(points_3d_dict, title, annotation_dir=annotation_dir_str, enable_video=True)
