...
class GMINDDataset(Dataset):
    ...
    def __init__(
        ...
        if len(self.video_items) == 0:
            raise ValueError(
                f"No videos found for sensor '{sensor}' in sets {sets} "
                f"at {data_root}"
            )
            ...
            for subdir in sorted(set_dir.iterdir()):
                ...
                if not video_files:
                    ...
                    if alt_videos:
                        ...
                        logger.debug(f"    Found {len(alt_videos)} video(s) with alternative naming")
                else:
                    logger.debug(f"    Found {len(video_files)} video(s) matching pattern '{video_pattern}'")
            ...
            for frame_idx in all_frame_indices:
                ...
                if image_id is None:
                    ...
                    if missing_annotations_count <= 5:  # Only log first few
                        logger.debug(f"Frame {frame_idx} not found in JSON frame_to_image_id mapping")
                        logger.debug(f"  Available frames in mapping: {sorted(list(frame_to_image_id.keys()))[:10]}...")
                ...
                frame_index.append({
                    ...
                    'image_info': next(
                        (img for img in ann_data.get('images', []) if img['id'] == image_id),
                        None
