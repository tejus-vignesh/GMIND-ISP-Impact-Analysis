"""
Infinity Projection Error Analysis Tool

Analyses and visualises projection errors at infinity for camera calibration validation.
This tool is essential for validating the accuracy of multi-camera calibration by
computing how well rays from different cameras converge at infinity points.

The tool:
1. Computes projection errors for points at infinity across multiple cameras
2. Visualises error patterns and distributions
3. Generates comprehensive error statistics and plots
4. Validates calibration accuracy for sensor fusion applications
...
def parse_sensor_calibration(calib_path: str) -> Tuple[Dict, Dict, Dict]:
    ...
    extrinsics, and image dimensions for all cameras defined in the file.
    ...
    Returns:
        Tuple containing:
            ...
            - extrinsics: Dictionary mapping camera name to extrinsics dict
                        with keys 'X', 'Y', 'Z' (translation) and 'R' (3x3 rotation matrix)
            - image_dimensions: Dictionary mapping camera name to (width, height) tuple
    """
    cameras = {}
    extrinsics = {}
    image_dimensions = {}
...
def transform_ray_to_camera_coords(
    ray_cam1: np.ndarray, cam1_extr: Dict, cam2_extr: Dict
) -> np.ndarray:
    """
    Transform a ray (direction vector) from camera1 coordinate system to camera2 coordinate system.

    Transforms a direction vector representing a ray in 3D space from one camera's
    coordinate frame to another. This is used to compute where rays from different
    cameras intersect at infinity.

    Args:
        ray_cam1: Normalized direction vector in camera1 frame (3-element numpy array)
        cam1_extr: Extrinsics dict for camera1 with 'R' (3x3 rotation matrix) and
                  'X', 'Y', 'Z' (translation components)
        cam2_extr: Extrinsics dict for camera2 with same structure as cam1_extr

    Returns:
        Normalized direction vector in camera2 frame (3-element numpy array)
    """
...
def calculate_infinity_error(
    ...
    flir_height,
    grid_size=20,
    test_distances=[4, 100],
):
    """
    Calculate the error (delta) caused by assuming objects are at infinity.

    For each grid point in FLIR at a given distance:
    1. Project using infinity assumption: get direction ray, transform to target camera, project with large distance
    2. Project using actual distance: get 3D point at actual distance, transform to target camera, project
    3. Calculate pixel error (delta) between the two projections

    Args:
        flir_camera: CameraModel for FLIR 8.9MP
        target_camera: CameraModel for target camera (e.g., FLIR 3.2MP)
        flir_extr: Extrinsics dict for FLIR
        target_extr: Extrinsics dict for target camera
    ...
    logger.info(f"Created {len(grid_points_flir)} grid points in FLIR image")

    # Convert pixels to rays in FLIR camera frame
    rays_flir = flir_camera.pixel_to_ray(grid_points_flir)  # (N, 3)
    logger.info(f"Converted {len(rays_flir)} pixels to rays")

    # Transform rays from FLIR to target camera frame (for infinity assumption)
    rays_target_infinity = np.array(
        [transform_ray_to_camera_coords(ray, flir_extr, target_extr) for ray in rays_flir]
    )

    # Project rays at infinity to target camera image
    infinity_pixels = []
    for ray in rays_target_infinity:
        if ray[2] > 0:  # Only project if pointing forward
            pixel = project_ray_at_infinity(target_camera, ray)
    ...
    for distance in test_distances:
        logger.info(f"Testing at distance: {distance}m")

        # Create 3D points in FLIR camera frame at the specified distance
        points_flir_3d = rays_flir * distance  # (N, 3)

        # Transform points from FLIR to target camera frame
        points_target_3d = np.array(
            [transform_point_to_camera_coords(p, flir_extr, target_extr) for p in points_flir_3d]
        )

        # Project to target camera image
        finite_pixels = []
        for i, point in enumerate(points_target_3d):
            if point[2] > 0:  # Only project if in front of camera
                pixel = target_camera.project_point(point.reshape(1, 3))[0]
        ...
        # Calculate
        # error delta (pixel distance between infinity and actual distance projections)
        ...
        if np.sum(valid_mask) > 0:
            ...
            logger.info(
                f"Valid points: {results[distance]['valid_points']}/{results[distance]['total_points']}"
            )
            logger.info(f"Mean error (delta): {results[distance]['mean_error']:.3f} pixels")
            logger.info(f"Median error (delta): {results[distance]['median_error']:.3f} pixels")
            logger.info(f"Max error (delta): {results[distance]['max_error']:.3f} pixels")
            logger.info(f"Std error (delta): {results[distance]['std_error']:.3f} pixels")
        else:
            logger.warning(f"No valid points at distance {distance}m")
...
def plot_debug_results(
    ...
    target_width,
    target_height,
    output_path="infinity_error_debug.png",
):
    """
    Create debug plots showing FLIR grid, target camera projections at infinity vs actual distances,
    ...
    # Create a grid layout: one row per distance, showing FLIR grid, target camera infinity, target camera finite, and error vectors
    ...
    for idx, distance in enumerate(distances):
        ...
        valid_infinity_target = r["infinity_pixels"]
        valid_finite_target = r["finite_pixels"]
        ...
        # Plot 2: Target camera projection at infinity
        ax2 = plt.subplot(n_distances, 4, idx * 4 + 2)
        ax2.set_xlim(0, target_width)
        ax2.set_ylim(target_height, 0)
        ax2.scatter(
            valid_infinity_target[:, 0],
            valid_infinity_target[:, 1],
            c="green",
            s=10,
            alpha=0.6,
            label="Infinity",
        ...
        ax2.set_title(
            f"FLIR 3.2MP: Infinity Projection\n({distance}m)", fontsize=12, fontweight="bold"
        )
        ...
        # Plot 3: Target camera projection at actual distance
        ax3 = plt.subplot(n_distances, 4, idx * 4 + 3)
        ax3.set_xlim(0, target_width)
        ax3.set_ylim(target_height, 0)
        ax3.scatter(
            valid_finite_target[:, 0],
            valid_finite_target[:, 1],
            c="red",
            s=10,
            alpha=0.6,
            label=f"Actual {distance}m",
        )
        ax3.set_title(
            f"FLIR 3.2MP: Actual Distance Projection\n({distance}m)", fontsize=12, fontweight="bold"
        )
        ...
        # Plot error vectors
        for i in range(len(valid_infinity_target)):
            if errors[i] > 0:
                start = valid_infinity_target[i]
                end = valid_infinity_target[i] + scaled_deltas[i]
        ...
        ax4.set_title(
            f'Error Vectors (Delta)\nMean: {r["mean_error"]:.3f}px', fontsize=12, fontweight="bold"
        )
...
def plot_error_delta_line(results_dict, output_path="infinity_error_delta.png"):
    """
    Create a standalone line plot showing max error delta vs distance for multiple targets.

    Args:
        results_dict: Dict mapping target name to results dict (e.g., {'CCTV': results_cctv, 'FLIR 3.2MP': results_flir32})
        output_path: Output file path for the plot
    """
    # Get all distances (use the first target's distances as reference)
    first_target = list(results_dict.keys())[0]
    distances = sorted(
        [d for d in results_dict[first_target].keys() if results_dict[first_target][d] is not None]
    )
    ...
    colors_styles = {"CCTV": ("blue", "o", "-"), "FLIR 3.2MP": ("red", "s", "--")}

    # Plot line for each target
    for target_name, results in results_dict.items():
        distances_target = sorted([d for d in results.keys() if results[d] is not None])
        max_errors = [results[d]["max_error"] for d in distances_target]

        color, marker, linestyle = colors_styles.get(target_name, ("green", "^", "-."))

        # Define darker edge color
        dark_colors = {"blue": "darkblue", "red": "darkred", "green": "darkgreen"}
        markeredge = dark_colors.get(color, "black")
    ...
    ax.set_xlabel("Distance (m)", fontsize=20, fontweight="bold")
    ax.set_ylabel("Max Reproj. Error (px)", fontsize=20, fontweight="bold")
    ax.set_title("Infinite Distance Approximation Error", fontsize=22, fontweight="bold")
    ...
    # Add vertical dashed line at 20m
    ax.axvline(x=20, color="gray", linestyle="--", linewidth=1.5, alpha=0.7, label="20m reference")
...
def main() -> None:
    """
    Main function to analyse infinity projection errors for calibration validation.

    Loads calibration data, computes projection errors at infinity for multiple
    camera pairs, and generates comprehensive visualisation plots and statistics.
    """
    ...
    # Get FLIR 8.9MP and target cameras (CCTV and FLIR 3.2MP)
    flir_name = "FLIR 8.9MP"
    cctv_name = "CCTV"
    flir32_name = "FLIR 3.2MP"
    ...
    if flir32_name not in cameras:
        print(f"Error: {flir32_name} not found in calibration file")
        print(f"Available cameras: {list(cameras.keys())}")
        return

    flir_camera = cameras[flir_name]
    cctv_camera = cameras[cctv_name]
    flir32_camera = cameras[flir32_name]
    flir_extr = extrinsics[flir_name]
    cctv_extr = extrinsics[cctv_name]
    flir32_extr = extrinsics[flir32_name]

    print(f"\nLoaded cameras:")
    print(f"  {flir_name}: {flir_extr['R']}")
    print(f"  {cctv_name}: {cctv_extr['R']}")
    print(f"  {flir32_name}: {flir32_extr['R']}")
    print(
        f"  FLIR 8.9MP translation: [{flir_extr['X']:.3f}, {flir_extr['Y']:.3f}, {flir_extr['Z']:.3f}]"
    )
    print(f"  CCTV translation: [{cctv_extr['X']:.3f}, {cctv_extr['Y']:.3f}, {cctv_extr['Z']:.3f}]")
    print(
        f"  FLIR 3.2MP translation: [{flir32_extr['X']:.3f}, {flir32_extr['Y']:.3f}, {flir32_extr['Z']:.3f}]"
    )
    ...
    if flir32_name in image_dimensions:
        flir32_width, flir32_height = image_dimensions[flir32_name]
    else:
        # Use defaults from sensor_calibration.txt
        flir32_width, flir32_height = 2048, 1536

    print(f"\nImage dimensions:")
    print(f"  {flir_name}: {flir_width}x{flir_height}")
    print(f"  {cctv_name}: {cctv_width}x{cctv_height}")
    print(f"  {flir32_name}: {flir32_width}x{flir32_height}")
    ...
    # Calculate errors for CCTV target
    print("\n" + "=" * 60)
    print("Calculating errors for CCTV target...")
    print("=" * 60)
    results_cctv, grid_points_flir, rays_flir, infinity_pixels_cctv = calculate_infinity_error(
        flir_camera,
        cctv_camera,
        flir_extr,
        cctv_extr,
    ...
    # Calculate errors for FLIR 3.2MP target
    print("\n" + "=" * 60)
    print("Calculating errors for FLIR 3.2MP target...")
    print("=" * 60)
    results_flir32, _, _, infinity_pixels_flir32 = calculate_infinity_error(
        flir_camera,
        flir32_camera,
        flir_extr,
        flir32_extr,
    ...
    # Also calculate for the key distances (4m, 20m, 100m) for debug plots (using FLIR 3.2MP)
    results_debug, _, _, infinity_pixels = calculate_infinity_error(
        flir_camera,
        flir32_camera,
        flir_extr,
        flir32_extr,
    ...
    # Generate standalone error delta line plot (max error) with both targets
    delta_output_path = os.path.join(output_dir, "infinity_error_delta.png")
    print("\n" + "=" * 60)
    print("Generating error delta line plot (max error) with both targets...")
    print("=" * 60)
    results_dict = {"CCTV": results_cctv, "FLIR 3.2MP": results_flir32}
    plot_error_delta_line(results_dict, delta_output_path)
    ...
    # Print summary for both targets
    for target_name, results in results_dict.items():
        print("\n" + "=" * 60)
        print(f"SUMMARY - {target_name}: Error Delta: Infinity Assumption vs Actual Distance")
        print("=" * 60)
        distances = sorted([d for d in results.keys() if results[d] is not None])

        # Show summary for key distances
        for distance in [5, 20, 50, 100]:
            if distance in results and results[distance] is not None:
                r = results[distance]
                print(f"\nDistance: {distance}m")
        ...
        # Compare distances
        if 5 in results and results[5] is not None and 100 in results and results[100] is not None:
            print("\n" + "-" * 60)
            print(f"COMPARISON ({target_name}): Close (5m) vs Far (100m)")
            print("-" * 60)
            close_error = results[5]["mean_error"]
            far_error = results[100]["mean_error"]
            print(f"5m mean error:  {close_error:.3f} pixels")
            print(f"100m mean error: {far_error:.3f} pixels")

            if close_error > 0:
                error_reduction = (close_error - far_error) / close_error * 100
                print(f"Error reduction from 5m to 100m: {error_reduction:.1f}%")
