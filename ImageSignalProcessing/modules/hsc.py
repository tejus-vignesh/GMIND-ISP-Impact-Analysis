...
class HSC(BasicModule):
    ...
    def execute(self, data):
        ...
        hsc_cb_image = np.right_shift(
            self.cos_hue * (cb_image - 128) - self.sin_hue * (cr_image - 128), 8
        )  # x256
        hsc_cb_image = np.right_shift(self.saturation_gain * hsc_cb_image, 8) + 128

        hsc_cr_image = np.right_shift(
            self.sin_hue * (cb_image - 128) + self.cos_hue * (cr_image - 128), 8
        )  # x256
