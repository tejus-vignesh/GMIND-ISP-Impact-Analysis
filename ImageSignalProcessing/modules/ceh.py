...
class CEH(BasicModule):
    def __init__(self, cfg):
        ...
        self.luts = np.empty(
            shape=(self.y_tiles, self.x_tiles, 256), dtype=np.uint8
        )  # LUTs w.r.t. tiles
    ...
    def execute(self, data):
        ...
        for ty in range(self.y_tiles):
            for tx in range(self.x_tiles):
                y_tile = y_image[
                    ty * self.tile_height : (ty + 1) * self.tile_height,
    ...
    def _is_corner_block(self, ix, iy):
        ...
        return (
            (iy == 0 and ix == 0)
            or (iy == 0 and ix == self.x_tiles)
            or (iy == self.y_tiles and ix == 0)
            or (iy == self.y_tiles and ix == self.x_tiles)
        )
