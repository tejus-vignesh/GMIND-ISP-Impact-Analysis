...
class CFA(BasicModule):
    ...
    def rotate_to_rggb(self, array):
        k = {"rggb": 0, "bggr": 2, "grbg": 1, "gbrg": 3}[self.cfg.hardware.bayer_pattern]
        return np.rot90(array, k=k)

    def rotate_from_rggb(self, array):
        k = {"rggb": 0, "bggr": 2, "grbg": 3, "gbrg": 1}[self.cfg.hardware.bayer_pattern]
        return np.rot90(array, k=k)
    ...
    def execute_malvar(self, data):
        ...
        g_on_r = np.right_shift(
            ...
            + self.index_weighted_sum(shifted_gb, *self.channel_indices_and_weights[5]),
            10,
        )
        b_on_r = np.right_shift(
            self.index_weighted_sum(shifted_r, *self.channel_indices_and_weights[1])
            + self.index_weighted_sum(shifted_b, *self.channel_indices_and_weights[13]),
            10,
        )
        ...
        r_on_gr = np.right_shift(
            ...
            + self.index_weighted_sum(shifted_gb, *self.channel_indices_and_weights[15]),
            10,
        )
        ...
        r_on_gb = np.right_shift(
            ...
            + self.index_weighted_sum(shifted_gb, *self.channel_indices_and_weights[3]),
            10,
        )
        b_on_gb = np.right_shift(
            self.index_weighted_sum(shifted_b, *self.channel_indices_and_weights[12])
            + self.index_weighted_sum(shifted_gr, *self.channel_indices_and_weights[16])
            + self.index_weighted_sum(shifted_gb, *self.channel_indices_and_weights[2]),
            10,
        )
        ...
        r_on_b = np.right_shift(
            ...
            + self.index_weighted_sum(shifted_b, *self.channel_indices_and_weights[1]),
            10,
        )
        g_on_b = np.right_shift(
            ...
            + self.index_weighted_sum(shifted_b, *self.channel_indices_and_weights[0]),
            10,
        )
